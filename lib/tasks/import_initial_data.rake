...
namespace :import_initial_data do
  ...
  task prefectures: :environment do
    path = Rails.root.join('assets/csv/prefectures.csv')
    relative_path = ".#{path}"
    list = []
  ...
  task cities: :environment do
    path = Rails.root.join('assets/csv/cities.csv')
    list = []
  ...
  task companies_shops: :environment do
    path = Rails.root.join('assets/csv/shops.csv')
    company_list = []
    relative_path = ".#{path}"
    CSV.foreach(relative_path, headers: true) do |row|
      company_list << {
    ...
    company_list.each do |company_hash|
      ...
      shop_list.each do |shop_hash|
        if company.name == shop_hash.keys[0]
  ...
  task reviews: :environment do
    path = Rails.root.join('assets/csv/reviews.csv')
    relative_path = ".#{path}"
    CSV.foreach(relative_path, headers: true) do |row|
