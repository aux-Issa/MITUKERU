class AssessmentRequest
    ...
    validates :property_city, presence: true, exclusion: { in: [0], message: "を選択して下さい" }
    validates :property_address, presence: true, length: {maximum: 50}
    validates :property_type, inclusion: {in: [1, 2, 3], message: "を選択して下さい"}
    ...
    validates :property_building_area_unit, inclusion: {in: [1, 2], message: "を選択して下さい"}
    validates :property_floor_area, presence: true
    validates :property_room_plan, inclusion: {in: (1..13).to_a, message: "を選択して下さい"}
    ...
    def property_city_choices shop
      # ToDo: データベースに無駄にアクセス
      choices = [["選択して下さい", 0]]
      shop.assessable_areas.map { |assessable_area| choices.push([City.find(assessable_area.city_id).prefecture.name + City.find(assessable_area.city_id).name, assessable_area.city_id])}
      return choices
    end

    def property_type_choices
      [["選択して下さい", 0], ["分譲マンション", 1], ["一戸建て", 2], ["土地", 3]]
    end

    def property_building_area_unit_choices
      [['選択して下さい', 0], ['平方メートル', 1], ['坪', 2]]
    end

    def property_room_plan
      [['選択して下さい', 0], ['1R(ワンルーム)', 1], ['1K / 1DK', 2], ['1LK / 1LDK', 3], ['2K / 2DK', 4], ['2LK / 2LDK', 5], ['3K / 3DK', 6], ['3LK / 3LDK', 7], ['4K / 4DK', 8], ['4LK / 4LDK', 9], ['5K / 5DK', 10], ['5LK / 5LDK', 11], ['6K / 6DK', 12], ['6LK / 6LDK以上', 13]]
    end
  end
